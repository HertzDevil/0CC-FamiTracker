...
public:
	...
	bool AddBookmark(std::unique_ptr<CBookmark> pMark);
	...
	bool SetBookmark(unsigned Index, std::unique_ptr<CBookmark> pMark);
	/*!	\brief Inserts a bookmark to the collection at a given position.
		\param Index Position index.
		\param pMark Pointer to the bookmark.
		\return Whether the bookmark is added. */
	bool InsertBookmark(unsigned Index, std::unique_ptr<CBookmark> pMark);
