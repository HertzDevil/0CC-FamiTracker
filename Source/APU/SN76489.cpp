...
CSNSquare::CSNSquare(CMixer &Mixer, chan_id_t ID) :
	CSN76489Channel(Mixer, sound_chip_t::SN76489, ID)
{
}
...
double CSNSquare::GetFrequency() const {
	return (double)CAPU::BASE_FREQ_NTSC / GetPeriod() / 32;
}
...
CSNNoise::CSNNoise(CMixer &Mixer) :
	CSN76489Channel(Mixer, sound_chip_t::SN76489, chan_id_t::SN76489_NOISE),
	m_iCH3Period(0)
{
...
double CSNNoise::GetFrequency() const {
	...
	return (double)CAPU::BASE_FREQ_NTSC / Period / 32;
}



CSN76489::CSN76489(CMixer &Mixer) : CSoundChip(Mixer),
	m_pChannels {
		std::make_unique<CSNSquare>(Mixer, chan_id_t::SN76489_CH1),
		std::make_unique<CSNSquare>(Mixer, chan_id_t::SN76489_CH2),
		std::make_unique<CSNSquare>(Mixer, chan_id_t::SN76489_CH3),
		std::make_unique<CSNNoise>(Mixer),
	}
{
}
...
//			m_pVGMWriter->WriteReg(0, Value, 0x06);
	...
	default:
		...
		if (m_iAddressLatch / 2 == GetChannelSubIndex(chan_id_t::SN76489_CH3))
			UpdateNoisePeriod();
	}

	if (Address / 2 == GetChannelSubIndex(chan_id_t::SN76489_CH3))
		UpdateNoisePeriod();
}
...
{
	Assert(m_pChannels[GetChannelSubIndex(chan_id_t::SN76489_NOISE)] != nullptr);
	return static_cast<CSNNoise *>(m_pChannels[GetChannelSubIndex(chan_id_t::SN76489_NOISE)].get());
}

void CSN76489::UpdateNoisePeriod() const
{
	GetNoise()->CachePeriod(GetSquare(GetChannelSubIndex(chan_id_t::SN76489_CH3))->GetPeriod());
}
